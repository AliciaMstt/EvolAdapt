...
Likelihood_Function <- ecdf(wolves_neutral$alpha)
```
...
```{r}
# run function
x<-Likelihood_Function(wolves_genic$alpha)
...
**Ejercicio:** Divídanse por equipos. Escriban una función que corra desde la función `ecdf()` hasta crear la df de resultados (ie todo lo anterior menos los plots). Los argumentos mínimos deben ser: 
...
Puedes ver un ejemplo de la función en en el script [outliers_pval.R](outliers_pval.R).

Para cargarla dentro de tu script de R:

```{r}
source("outliers_pval.R")
```

Y luego puede correrse así para los datos de BayeScan:

```{r}
# Load Bayescan output data
...
# Get pvalues
outliers_pval(neutraldata=wolves_neutral, genicdata = wolves_genic,
              desiredcolumn = 5, #alpha is the 5th column
              method= "BH")
